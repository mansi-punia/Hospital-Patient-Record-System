...
public class FileManager {
    ...
    public static List<Patient> loadPatientsJson(Path path) throws IOException {
        ...
        try (Reader r = Files.newBufferedReader(path)) {
            ...
            // we deserialize manually because of polymorphism
            JsonArray arr = JsonParser.parseReader(r).getAsJsonArray();
            List<Patient> list = new ArrayList<>();
            for (JsonElement el : arr) {
                JsonObject obj = el.getAsJsonObject();
                String type = obj.get("className") != null ? obj.get("className").getAsString() : null;
                // fallback: inspect keys
                if (obj.has("admitDate") || obj.has("assignedBedId")) {
                    // InPatient
                    Patient p = gson.fromJson(obj, InPatient.class);
                    list.add(p);
                } else if (obj.has("visitDate")) {
                    Patient p = gson.fromJson(obj, OutPatient.class);
                    list.add(p);
                } else {
                    // generic - parse as Patient? We'll ignore unknown
                }
            }
            return list;
        } catch (Exception e) {
    ...
    {
        try (Writer w = Files.newBufferedWriter(path)) { gson.toJson(doctors, w); }
    }
